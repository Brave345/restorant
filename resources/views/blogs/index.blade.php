  ...
  <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
    <div class="sidenav-header">
      ...
      <a class="navbar-brand m-0" href=" https://demos.creative-tim.com/argon-dashboard/pages/dashboard.html " target="_blank">
        ...
        <span class="ms-1 font-weight-bold">Admin Blog</span>
      </a>
    </div>
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="blogs">
            <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder text-white mb-0">Tables Blog</h6>
        </nav>
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr>
                    ...
                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tema Cooking</th>
...
<tbody>
    ...
    @else
        ...
        @foreach ($blogs as $blog)
            <tr>
                <td>
                    <p class="text-xs font-weight-bold mb-0">{{ $blog->user }}</p>
                </td>
                <td>
                    <p class="text-xs font-weight-bold mb-0">{{ $blog->judul }}</p>
                </td>
                <td>
                    <p class="text-xs text-secondary mb-0">{{ $blog->isi }}</p>
                </td>
                 <td>
                    <p class="text-xs text-secondary mb-0">{{ $blog->tanggal }}</p>
                </td>
                <td class="align-middle text-center">
                    <img src="{{ asset('assets/images/' . $blog->gambar) }}" alt="Gambar Artikel" class="img-fluid">
                </td>
                <td>
                    <p class="text-xs font-weight-bold mb-0">{{ $blog->tema_cooking }}</p>
                </td>
                <td class="align-middle">
                    <a href="{{ route('blogs.edit', $blog->id) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit Artikel">
                        Edit
                    </a>
                    <form action="{{ route('blogs.destroy', $blog->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-secondary font-weight-bold text-xs" onclick="return confirm('Apakah Anda yakin ingin menghapus Blog ini?')" data-toggle="tooltip" data-original-title="Hapus Artikel">
                            Hapus
                        </button>
                    </form>
                </td>
